...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    {{Form::open(["url"=>"/admin/women/cateogry/delete/"])}}
    <!-- Main content -->
    <section class="content">
        <h3 class="bg-danger text-center" style="padding: 5px 0px;">Caution: Deleting any Category will also delete all product data.</h3>
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">{{ isset($type) ? $type : ""}}</h3>
              <a href="{{url('admin/women/category/add')}}" class="btn btn-primary pull-right">Add New</a>
              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                @if(count($women)>0)
                  @foreach($women as $item)
                      <tr>
                        <td><input type="checkbox" name="IDs[]" value="{{ $item->CategoryID }}" class="lowercheckbox"></td>
                        <td>{{ $item->CategoryID }}</td>
                        <td>{{ $item->CategoryName }}</td>
                        <td>{{ $item->Status }}</td>
                        <td>{{ $item->DateAdded }}</td>
                        <td>{{ $item->DateModified }}</td>
                        <td><a href="{{url('admin/women/category').'/'. $item->CategoryID.'/edit'}}" class="btn btn-danger">Edit</a></td>
                      </tr> 
                  @endforeach   
                @endif
