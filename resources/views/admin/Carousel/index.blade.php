...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    {{Form::open(["url"=>"/admin/carousel/delete/"])}}
    <!-- Main content -->
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              ...
              <a href="{{url('admin/carousel/add')}}" class="btn btn-primary pull-right">Add New</a>
              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                @if(count($carousel)>0)
                  @foreach($carousel as $item)
                      <tr>
                        <td><input type="checkbox" name="IDs[]" value="{{ $item->CarouselID}}" class="lowercheckbox"></td>
                        <td>{{ $item->CarouselID }}</td>
                        <td>{{ $item->Heading }}</td>
                        <td>{{ $item->Status }}</td>
                        <td>{{ $item->DateAdded }}</td>
                        <td>{{ $item->DateModified }}</td>
                        <td><a href="{{url('admin/carousel/').'/'.$item->CarouselID.'/edit'}}" class="btn btn-danger">Edit</a></td>
                      </tr> 
                  @endforeach   
                @endif
