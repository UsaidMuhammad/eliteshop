		...
		$(document).ready(function () {
			...
			*/
			...
			function LoadEverything(value) {
				$.get("{{url('/womens/'.$title[1].'/getJSON')}}", function (data) {
					...
					for (var i = 0; i < data.length; i++) {
						...
						$products += '<a href=\'{{url("/item")}}/'+data[i].ProductID+'/women\' class="link-product-add-cart">Quick View</a>';
						$products += "</div>";
						$products += '</div>';
						$products += "</div>";
						$products += '<div class="item-info-product ">';
						$products += '<h4>';
						$products += '<a href=\'{{url("/item")}}/'+data[i].ProductID+'/women\'>'+data[i].ProductName+'</a>';
						$products += '</h4>';
